...
export function lotScoreFromAcres(acres) {
...
/**
 * Combine the size, AI, and highway sub-scores using the canonical
 * 50/35/15 weighting. Returns null if `aiScore` is null/undefined
 * (no AI assessment yet means the composite is undefined).
 * `highwayScore` is treated as 0 when null/undefined.
 */
export function recomputeComposite({ acres, aiScore, highwayScore }) {
  if (aiScore === null || aiScore === undefined) return null;
  const lotScore = lotScoreFromAcres(acres);
  const hwy = highwayScore ?? 0;
  return Math.round(aiScore * 0.35 + lotScore * 0.50 + hwy * 0.15);
}
...
export async function scoreVenue({ venueId, lat, lng, name }) {
  ...
  const baseComposite = recomputeComposite({
